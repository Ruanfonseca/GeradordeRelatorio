...
@CrossOrigin("*")
public class RabbitConsumerRelatorioSALA {


    @Autowired
    private RelatorioSalaService service;

    @RabbitListener(queues = "RELATORIO_SALA")
    ...
    public void processarLista(TemplateMSG msg){
        ...
        List<OrdemDeServicoSalaMongo> listaDeOS = new ArrayList<OrdemDeServicoSalaMongo>();
        ...
        if (msg != null){
            service.todasOSsala(datainicial,datafinal).forEach(listaDeOS::add);
        }
    ...
    @GetMapping("/SALA_PDF")
    public ResponseEntity<byte[]> geradorDeRelatorio(List<OrdemDeServicoSalaMongo> listaDeOS) {
        try {
